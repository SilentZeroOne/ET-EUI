...
namespace ET
{
    public class C2M_EquipItemHandler: AMActorLocationRpcHandler<Unit, C2M_EquipItem, M2C_EquipItem>
    {
        protected override async ETTask Run(Unit unit, C2M_EquipItem request, M2C_EquipItem response, Action reply)
        {
            ...
            if (!bagComponent.IsItemExit(request.ItemId))
            {
            ...
            Item equipItem = equipmentsComponent.GetEquipItemByPosition((int)equipPosition);
            if (equipItem != null)
            {
                ...
                equipItem = equipmentsComponent.UnLoadEquipItemByPosition((int) equipPosition);
