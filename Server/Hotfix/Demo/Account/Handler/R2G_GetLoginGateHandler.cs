...
namespace ET
{
    public class R2G_GetLoginGateHandler : AMActorRpcHandler<Scene,R2G_GetLoginGateKey,G2R_GetLoginGateKey>
    {
        protected override async ETTask Run(Scene scene, R2G_GetLoginGateKey request, G2R_GetLoginGateKey response, Action reply)
        {
            ...
            string key = TimeHelper.ServerNow().ToString() + RandomHelper.RandInt64();
